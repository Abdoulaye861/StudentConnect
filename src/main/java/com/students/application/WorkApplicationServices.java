...
public class WorkApplicationServices {
    ...
    public WorkDTO setMark(Long workid, Double markValue) {
        ...
        work.getMark().setMarkValue(markValue);
        work.getMark().setEvaluator(work.getCourse().getPublisher());
        workRepository.save(work);
        return mapper.map(work, WorkDTO.class);
    }


    public List<WorkAndMarkDTO> getAllStudentWorksForStudent(Long studentid) {
        List<WorkAndMarkDTO> works = new ArrayList<>();
        Student student = studentRepository.findOne(studentid);
        if (student.getWorks() != null) {
            for (Work work : student.getWorks()) {
